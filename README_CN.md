...
[**快速开始**](#快速开始) · [**工作原理**](#工作原理) · [**为什么要用 Rust 重写？**](#为什么要用-rust-重写) · [English](README.md)

</div>

**[RepoWiki](https://github.com/he-yufeng/RepoWiki) 的 Rust 移植版** — 从终端或浏览器为任意代码仓库生成完整 wiki 文档。单个静态二进制，无运行时，无 Docker。

原版 RepoWiki 是 [he-yufeng](https://github.com/he-yufeng) 的 Python 项目。本项目是从零开始的 Rust 重写，采用索引驱动的分析架构——每个模块的 LLM token 开销降低约 85%，同时输出与原版相同的结构化 wiki。

本仓库的 `wiki/` 目录就是用 `repowiki scan . --site` 扫描自身代码生成的——吃自己的狗粮。

## 为什么要用 Rust 重写？

Python 原版已经很好用了，但它需要 Python 运行时、拉进 `litellm` 和一堆依赖，而且直接把原始源码喂给 LLM。这个移植版解决了这三个问题：

| | 原版 (Python) | **本移植版 (Rust)** |
|---|---|---|
| 安装 | `pip install` + Python 运行时 | **单个二进制，无运行时** |
| 依赖 | litellm, FastAPI, uvicorn, ... | **零运行时依赖** |
| LLM 输入 | 原始源码文件（~10K tokens/模块） | **结构化索引摘要（~1.5K tokens/模块）** |
| import 解析 | 2 种语言（Python, JS/TS） | **6 种语言（Python, JS/TS, Go, Rust, Java, C/C++）** |
| 二进制大小 | N/A | **~15 MB 静态二进制** |

相同的 wiki 输出、相同的 CLI 接口、相同的三种导出格式——只是更快、更小、更省钱。

## 快速开始

```bash
# 从源码构建（尚未发布到 crates.io）
...
- **索引驱动分析** — 确定性的预索引（符号、import、调用图）为 LLM 提供紧凑的结构化摘要，而非直接扔原始源码；每个模块的 token 开销降低约 85%，同时描述仍基于真实代码。这是与 Python 原版最主要的架构差异。
...
- **增量重跑**：输出目录里的 `.repowiki-state.json` 记录每个页面由哪些输入生成，再次扫描只重新生成源码有变化的页面，并清理被删模块对应的页面；JSON 和 HTML 导出在内容没有变化时直接不写盘。加 `--full` 可强制全量重建。没变的页面连 LLM 调用都省掉：分析结果存在按内容寻址的 SQLite 缓存里（`~/.repowiki/cache.db`），小改之后重扫对没动的模块零 API 调用。
...
核心流水线已端到端跑通：扫描、索引、分析、三种格式导出、Web UI、终端问答。索引驱动分析架构是相对 Python 原版的关键改进。接下来：

- **发布到 crates.io** — 在几个真实仓库上验证 wiki 输出质量与 Python 原版对齐后发布。
- **更多图表类型**：在依赖图之外再加调用图和数据流图——分析本来就走了 import，能挖出更多。
- **并行文件读取** — 扫描器目前是单线程的；索引和分析阶段已经并行化。

## 致谢

- [**he-yufeng/RepoWiki**](https://github.com/he-yufeng/RepoWiki) — 原版 Python 项目。相同的 CLI 接口、相同的 wiki 输出格式、相同的理念。这个移植版存在，是因为这套架构值得一个更快、更精简的实现。
